...
struct loginBox: View {
    @EnvironmentObject private var loc: AppLocalizationManager  
    @StateObject var viewModel: Login_ViewModel
    @State private var rememberMe = false
    @State private var showPassword = false
            ...
            VStack {
                ...
                HStack {
                    ...
                    TextField(
                        ...
                        prompt: Text(loc.localized("email_placeholder"))
                    )
                    ...
                    Group {
                        if showPassword {
                            TextField(
                                ...
                                prompt: Text("password_placeholder")
                            )
                        } else {
                            SecureField(
                                "", text: $viewModel.password,
                ...
                .navigationDestination(isPresented: $viewModel.isLoggedIn) { MainTabView() }
                .padding(.horizontal)
                .padding(.bottom, 10)

                // Error Message (consider localizing error text in your ViewModel)
                if let error = viewModel.errorMessage {
                    Text(error) // or Text(LocalizedStringKey(errorKey)) if you return keys
                        .foregroundColor(.red)
                        .font(.footnote)
                        .padding(.bottom, 10)
                }
